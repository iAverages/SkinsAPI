module.exports = (req, _, next) => {
    ...
    // If neither are set, defaults to -1
    // When set to -1, the routes set their own defaults.
    const width = Math.min(1000, check(q?.width) ? +q.width : check(q?.height) ? +q.height : -1);
