 ...
 */
const uwu = (message) => {
    logger(`${colors.magenta("[UWU]")} ${message}`);
};
 ...
 */
const info = (message) => {
    logger(`${colors.cyan("[INFO]")} ${message}`);
};
...
const success = (message) => {
    logger(`${colors.green("[SUCCESS]")} ${message}`);
};
 ...
 */
const warn = (message) => {
    logger(`${colors.yellow("[WARN]")} ${message}`);
};
 ...
 */
const setup = (message) => {
    logger(`${colors.cyan("[SETUP]")} ${message}`);
};
...
const error = (message, ping = true) => {
    ...
    if (ping && webhook) {
        try {
            axios.post(webhook, { content: `[SKINAPI] <@307952129958477824> ${message}` });
        } catch (err) {
